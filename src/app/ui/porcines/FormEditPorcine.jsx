...
export default function FormEditPorcine({ id, porcine, onCancel }) {
    // Hooks
    const { handleEditPorcine } = usePorcines();
    ...
    const {
        ...
        defaultValues: {
            breed: porcine?.breed,
            age: porcine?.age,
            weight: porcine?.weight,
            client: porcine?.client?.id,
            feed: porcine?.feed?.id,
        },
    });
    ...
    const onSubmit = (data) => {
        ...
        handleEditPorcine(id, refactoredData)
            .then(() => {
